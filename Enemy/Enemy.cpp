...
void Enemy::Back() {
	MoveTimer--;
	worldTransform_.translation_ += MyMath::vector3Normalize(MyMath::SubVec(MovePhase, worldTransform_.translation_)) * 0.2f;
	worldTransformEnemyHp_.translation_ += MyMath::vector3Normalize(MyMath::SubVec(MovePhase, worldTransform_.translation_)) * 0.2f;
	if (MoveTimer < 0) {
