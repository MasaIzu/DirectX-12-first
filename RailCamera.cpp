...
void RailCamera::Initialize(WinApp* winApp,const Vector3 pos, const Vector3 rad) {
	//ワールドトランスフォームの初期設定
...
#pragma endregion
	...
	//レールカメラの回転を反映
	forward = MyMath::MatVector(worldTransform_.matWorld_, forward);
	//視点から前方に適当な距離進んだ位置が注視点
	viewProjection_.target = viewProjection_.eye + forward;
	Target = MyMath::vector3Normalize(viewProjection_.target);
	//ワールド上方ベクトル
	Vector3 up(0, 1, 0);
	//レールカメラの回転を反映(レールカメラの上方ベクトル)
	viewProjection_.up = MyMath::MatVector(worldTransform_.matWorld_, up);
